...
### PR3.1 - Exercicis lliurables

Els scripts demanats a la memòria estan dins de `exemple-mongodb/src`:

- `exercici1.js`: llegeix `Posts.xml`, selecciona les 10000 preguntes amb més `ViewCount`, decodifica entitats HTML, escriu logs a `data/logs/exercici1.log` i insereix documents a MongoDB.
- `exercici2.js`: executa les 2 consultes sobre MongoDB, mostra el nombre de coincidències per consola i genera `data/out/informe1.pdf` i `data/out/informe2.pdf`.
...
Fitxer XML d'entrada esperat per defecte:

- `data/Posts.xml` (configurable amb `STACKEXCHANGE_XML_PATH` a `exemple-mongodb/.env`)
